...
import { ObjectLike, RecursiveAccessKeyOf } from "./types";
import { isNil, isNotNil } from "./existence";
...
export const unique = <T, K extends string | number | symbol>(
  ...
  if (is<Array<string | number | symbol>>(array)) return Array.from(new Set(array)) as T[];
  if (!toIdentity) throw new Error('toIdentity is required when array is not an array of strings, numbers, or symbols')

  const values: T[] = [];
  array.reduce((acc, item) => {
    const identity = toIdentity(item)
    if (acc.includes(identity)) return acc
    acc.push(identity)
    values.push(item)
    return acc
